...
export default function Nav() {
  ...
  const watchPage = location.pathname === `/watch` || location.pathname === "/studio/stream-manager";
    ...
    <div className={`w-full sticky top-0 z-10 ${watchPage ? "bg-[#212121]" : "bg-w"}`}>
      <div className="flex items-center max-w-[1450px] mx-auto justify-between px-4 py-4 backdrop-blur-sm">
        <div className="flex items-center">
          ...
          <ul className="flex items-center gap-5 ml-12">
            {["Home", "Discover", "Watch", "Messages"].map((item) => (
              <li className={`${watchPage ? "text-white" : "hover:text-blue-500"}  text-[20px] mt-[3px]`} key={item}>
        ...
        <div className="flex items-center gap-2">
          <form onSubmit={handleSubmit} className="flex items-center bg-gray-100 rounded-3xl py-2 px-4">
            <div>
          ...
          <div className="cursor-pointer relative" onClick={handleClick}>
            <img className="h-[38px] w-[38px] rounded-full" src={existingUser.img} alt="user" />
            <div
            ...
            >
              {navItems.map((item) => (
                <div className="w-max rounded-sm hover:bg-gray-100 px-4 py-2 min-w-full " key={item.id}>
                  <Link
                    to={`${item.title === "Creator Studio" ? `/studio/content` : `/${item.title.toLowerCase()}`}`}
